        ...
        <div className="space-y-4">
          ...
          {!isExamPaper && (
            <div className="space-y-3 pt-2">
              ...
              <button
                type="button"
                onClick={() => setShowStudyDetails((open) => !open)}
                className="w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-left transition"
              >
                <span className="min-w-0">
                  <span className="block text-xs font-bold text-slate-800">
                    {showStudyDetails ? "收合" : "更多"}
                  </span>
                  {!showStudyDetails && (
                    <span className="block text-[11px] font-medium text-slate-500 truncate">
                      核心考點、易錯陷阱、詳細解題、解題技巧、心得
                    </span>
                  )}
                </span>
                <ChevronDown
                  className={`w-4 h-4 shrink-0 text-slate-400 transition-transform ${
                    showStudyDetails ? "rotate-180" : ""
                  }`}
                />
              </button>
                ...
                <div className="space-y-2.5">
                  {question.coreConcepts && (
                    <div className="p-3 rounded-xl bg-sky-50/80 border border-sky-200/80 text-xs">
                      <span className="font-bold text-sky-900 mb-0.5 flex items-center gap-1">
                        <Lightbulb className="w-3.5 h-3.5 text-sky-600" />
                        核心考點
                      </span>
                      <div className="text-sky-800 font-sans">
                  ...
                  {question.tips && (
                    <div className="text-[11px] text-indigo-700 bg-indigo-50/60 p-2.5 rounded-lg border border-indigo-100 flex items-start gap-1.5">
                  ...
                  <div className="p-3 rounded-xl bg-slate-50/60 border border-dashed border-slate-300 text-xs">
                    ...
                    {question.myNotes ? (
                      <div className="text-slate-600 italic font-sans">
