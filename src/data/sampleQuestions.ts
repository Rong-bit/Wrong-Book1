...
export const initialSampleQuestions: QuestionItem[] = [
  {
    ...
    questionText: "若關於 x 的一元二次方程式 x² - 2(k - 1)x + (k + 5) = 0 有兩相異實根，則實數 k 的取值範圍為何？\n(A) k > 4 或 k < -1\n(B) -1 < k < 4\n(C) k > 3 或 k < -2\n(D) -2 < k < 3",
    questionType: "單選題",
    answer: "(A) k > 4 或 k < -1",
    coreConcepts: "一元二次方程式 ax² + bx + c = 0 擁有兩相異實數根的充要條件為判別式 Δ = b² - 4ac > 0。",
    commonPitfalls: "常犯錯誤：\n1. 將判別式算式中的一次項係數誤代為 -(k-1) 忘記平方或漏乘 4ac。\n2. 解二次不等式 (k-4)(k+1) > 0 時，混淆不等號方向，誤寫成介於兩根之間 -1 < k < 4。",
    explanation: "1. 由題意，二次項係數 a = 1，一次項係數 b = -2(k - 1)，常數項 c = k + 5。\n2. 方程式有兩相異實根，故判別式 Δ > 0：\n   Δ = [-2(k - 1)]² - 4(1)(k + 5) > 0\n   4(k² - 2k + 1) - 4(k + 5) > 0\n3. 兩邊同除以 4：\n   k² - 2k + 1 - k - 5 > 0\n   k² - 3k - 4 > 0\n4. 因式分解得：\n   (k - 4)(k + 1) > 0\n5. 解此不等式得 k > 4 或 k < -1。\n故正確答案選 (A)。",
    tips: "化簡時善用「兩邊同除以 4」簡化計算，避免大數相乘；開口向上二次式大於零取兩外側（比大者大、比小者小）。",
    difficulty: "中等",
    myNotes: "第一次段考第 7 題粗心算錯。下次遇到有相異實根，立刻在題卷空白寫下 Δ > 0 再動筆！",
  ...
  },
  {
    ...
    questionText: "一質量為 m = 2 kg 的木塊靜止置於光滑水平桌面上，受到一恆定水平拉力 F = 10 N 持續作用，不計空氣阻力與接觸面摩擦。試問作用 4 秒末時，該木塊的動能 (Ek) 為多少焦耳 (J)？\n(A) 100 J\n(B) 200 J\n(C) 400 J\n(D) 800 J",
    questionType: "計算選擇題",
    answer: "(C) 400 J",
    coreConcepts: "牛頓第二運動定律 a = F/m；等加速度運動速度公式 v = v₀ + at；動能公式 Ek = 1/2 mv²（或動量定理 / 功能定理 W = F·s = ΔEk）。",
    commonPitfalls: "1. 誤將衝量 J = F·Δt (40 N·s) 當作動能。\n2. 動能 Ek = 1/2 mv² 忘記乘以 1/2 或忘記將速度平方。",
    explanation: "【解法一：牛頓定律與運動學】\n1. 由 a = F / m = 10 / 2 = 5 (m/s²)。\n2. 木塊初速 v₀ = 0，作用 4 秒末速度 v = v₀ + at = 0 + 5 × 4 = 20 (m/s)。\n3. 4 秒末木塊動能 Ek = 1/2 m v² = 1/2 × 2 × (20)² = 400 (J)。\n\n【解法二：功能定理（秒解）】\n物體在 4 秒內的位移 s = 1/2 a t² = 1/2 × 5 × 4² = 40 (m)。\n外力作功 W = F × s = 10 × 40 = 400 (J) = 動能變化量 ΔEk。\n故正確選項為 (C)。",
    tips: "考場上建議雙解法互相驗證：F·t 給出動量 p = 40，則 Ek = p² / (2m) = 1600 / 4 = 400 J，最快且不易出錯！",
    difficulty: "中等",
    myNotes: "熟記 Ek = p²/(2m) 轉換公式！",
